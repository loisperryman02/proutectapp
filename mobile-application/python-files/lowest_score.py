# Corrected and shortened version of the data for demonstration purposes
safety_scores = [
  {
    "Latitude": 52.295042,
    "Longitude": -1.570058,
...
df = pd.DataFrame(safety_scores)

# Group by coordinates and calculate the average safety score
avg_scores = df.groupby(['Latitude', 'Longitude']).mean().reset_index()

# Find the row with the lowest average safety score
lowest_avg_score = avg_scores.loc[avg_scores['Safety_Score'].idxmin()]
